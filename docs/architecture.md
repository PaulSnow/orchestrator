...
orchestrator/
  ...
  scripts/
    proof-workers/             tmux orchestration (primary execution model)
      tmux-orchestrate.sh      Launch full tmux session with workers
      tmux-worker-prompt.sh    Generate implementation prompts
      tmux-review-prompt.sh    Generate review/test prompts
...
1. `tmux-orchestrate.sh` creates worktrees from `config/proof-issues.json`
2. Each worker gets a prompt generated by `tmux-worker-prompt.sh` (implementation) or `tmux-review-prompt.sh` (review/test)
...
5. Monitor loop in window 0 detects the signal and assigns next issue
6. If a worker stalls (no log growth for 15 min), monitor kills and restarts with continuation context
